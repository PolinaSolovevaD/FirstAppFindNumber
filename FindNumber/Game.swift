...
//  FindNumber
//
//  Created by Polya Soloveva on 24/6/23.
//
...
class Game {
            ...
            if status != .start{
                if status == .win{
                    ...
                    if record == 0 || newRecord < record{
                        UserDefaults.standard.setValue(newRecord, forKey: KeysUserDefaults.recordsGame)
                        isNewRecord = true
                    }
                }
                stopGame()
    ...
    init(countItems: Int, updateTimer: @escaping (_ status: StatusGame, _ seconds: Int) -> Void) {
        ...
        self.timeForGame = Settings.shared.currentSattings.timeForGame
    ...
    private func setupGame() {
        ...
        if Settings.shared.currentSattings.timerState {
